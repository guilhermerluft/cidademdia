...
grep -q 'institutionId' "$CENTER" || fail "formulário não envia instituição selecionada"
...
DESTINATION_LINE="$(grep -n 'Destino institucional <span className="occurrence-required-marker"' "$CENTER" | head -n1 | cut -d: -f1)"
