...
DESCRIPTION_LINE="$(grep -n '^                Descrição
...
echo "occurrence_creation_institution_destination=OK"
echo "occurrence_creation_address_required=OK"
echo "occurrence_creation_protocol_required=OK"
echo "occurrence_creation_photo_required=OK"
echo "occurrence_creation_field_errors_scoped=OK"
...
echo "PUBLIC OCCURRENCE DETAILS GUARD: OK"
 "$CENTER" | head -n1 | cut -d: -f1)"
LOCATION_LINE="$(grep -n '<OccurrenceLocationPicker' "$CENTER" | head -n1 | cut -d: -f1)"
DESTINATION_LINE="$(grep -n 'Destinatário <span className="occurrence-required-marker"' "$CENTER" | head -n1 | cut -d: -f1)"
