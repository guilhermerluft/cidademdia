...
grep -q 'Guid? InstitutionId' "$CREATE_ENDPOINTS" || fail "criação não aceita destino institucional"
grep -q 'Exactly one destination must be selected' "$CREATE_ENDPOINTS" || fail "criação não exige um único destino"
...
grep -q 'listOccurrenceDestinations' "$CENTER" || fail "formulário não resolve destinatários pela localidade"
grep -q 'masterUserId: form.masterUserId || null' "$CENTER" || fail "formulário não envia Master selecionada"
grep -q 'institutionId: form.institutionId || null' "$CENTER" || fail "formulário não envia fallback institucional selecionado"
grep -q 'Nome ou partido (opcional)' "$CENTER" || fail "formulário não oferece endereçamento opcional"
...
grep -Fq 'public-occurrence-support__icon" aria-hidden="true">👍</span>' "$SUPPORT" || fail "apoio não usa emoji de polegar para cima"
grep -Fq 'Apoie essa ocorrência' "$SUPPORT" || fail "apoio não exibe o CTA textual esperado"
...
grep -q 'padding: 10px 210px 10px 0' "$CARD_CSS" || fail "conteúdo principal não reserva espaço para o CTA de apoio"
grep -q 'position: static' "$CARD_CSS" || fail "CTA de apoio não sai do posicionamento absoluto no mobile"
...
echo "occurrence_form_address_before_destination=OK"
echo "occurrence_creation_contextual_destination=OK"
...
echo "public_occurrence_support_cta=OK"
