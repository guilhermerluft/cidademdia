...
grep -Fq '"Câmara dos Deputados"' "$SERVICE" || fail "fallback Câmara dos Deputados ausente"
grep -Fq '"Senado Federal"' "$SERVICE" || fail "fallback Senado Federal ausente"
...
grep -Fq "Compartilhe com:" "$CENTER" || fail "formulário não mostra Compartilhe com"
grep -Fq "Informe a localização da ocorrência primeiro" "$CENTER" || fail "placeholder de localização do destino incorreto"
...
destination_line="$(grep -n -F 'Compartilhe com: <span' "$CENTER" | head -n1 | cut -d: -f1)"
...
test -n "$destination_line" || fail "campo Compartilhe com ausente"
...
grep -Fq 'DESTINATIONS: 7 GENERIC FALLBACKS' "$VISUAL_TEST" || fail "smoke visual não valida sete fallbacks genéricos"
...
grep -Fq 'fallback-camara-deputados.master@cidademdia.com.br' "$PROD_SEED" || fail "seed de produção não provisiona Câmara dos Deputados fallback"
grep -Fq 'fallback-senado.master@cidademdia.com.br' "$PROD_SEED" || fail "seed de produção não provisiona Senado fallback"
if grep -Fq '@hml.cidademdia.invalid' "$PROD_SEED"; then
  fail "seed de produção contém conta exclusiva de HML"
fi
grep -Fq 'HML_DEFAULT_OCCURRENCE_DESTINATIONS=OK count=7' "$HML_FALLBACK_SEED" || fail "seed HML não valida sete fallbacks"
grep -Fq 'HML_DEFAULT_OCCURRENCE_MASTERS=OK count=7' "$HML_FALLBACK_SEED" || fail "seed HML não valida sete Masters técnicas"
grep -Fq 'PRODUCTION_DEFAULT_OCCURRENCE_DESTINATIONS=OK count=7' "$PROD_SEED" || fail "seed de produção não valida os sete fallbacks"
grep -Fq 'PRODUCTION_DEFAULT_OCCURRENCE_MASTERS=OK count=7' "$PROD_SEED" || fail "seed de produção não valida as sete Masters técnicas"
