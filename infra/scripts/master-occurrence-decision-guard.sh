...
CHAT_PANEL="$ROOT/apps/web/src/modules/chat/ChatPanel.tsx"
...
test -f "$CHAT_PANEL" || fail "painel de chat não encontrado"
...
! grep -q 'type="file"' "$CHAT_PANEL" || fail "chat voltou a permitir seleção de arquivo de áudio"
! grep -q 'Selecionar áudio' "$CHAT_PANEL" || fail "chat ainda exibe seleção de arquivo de áudio"
grep -q 'fa-solid fa-microphone' "$CHAT_PANEL" || fail "composer não possui ação inline de microfone"
grep -q 'fa-solid fa-paper-plane' "$CHAT_PANEL" || fail "composer não possui ação inline de envio de texto"
grep -q 'chat-panel__composer-actions' "$CHAT_PANEL" || fail "microfone e envio não estão agrupados no composer"
grep -q "recording ? stopRecording" "$CHAT_PANEL" || fail "gravação não usa o mesmo botão para parar e enviar"
grep -q 'formatRecordingTime' "$CHAT_PANEL" || fail "gravação não apresenta feedback de duração"
...
echo "voice_recording_only=OK"
echo "inline_chat_composer=OK"
