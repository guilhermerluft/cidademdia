...
import { OccurrenceChatModal } from '../chat/OccurrenceChatModal';
import {
...
export function OccurrenceAssignmentPanel({ mode }: OccurrenceAssignmentPanelProps) {
  ...
  async function handleOpenDetails(target: MasterOccurrenceTarget) {
    setDetailsLoadingId(target.occurrenceId);
    setError(null);

    try {
      setSelectedOccurrence(await getPublicOccurrenceDetails(target.occurrenceId));
    } catch (requestError) {
  ...
  if (mode === 'master') {
    return (
      <>
        <section className="dashboard-section assignment-panel" id="dashboard-assignments">
          ...
          <div className="assignment-list">
            ...
            {masterTargets.map((target) => {
              ...
              const isLoadingDetails = detailsLoadingId === target.occurrenceId;

              return (
                <Card key={target.targetId} className="assignment-card">
                  <CardBody>
                    <div className="assignment-card__overview">
                      <button
                        type="button"
                        className="assignment-card__cover"
                        aria-label={`Ver ocorrência ${target.publicCode}`}
                        disabled={isLoadingDetails}
                        onClick={() => void handleOpenDetails(target)}
                      >
                        {target.coverMedia ? (
                          <img
                            src={target.coverMedia.readUrl}
                            alt=""
                            loading="lazy"
                            decoding="async"
                          />
                        ) : (
                          <span aria-hidden="true"><i className="fa-regular fa-image" /></span>
                        )}
                      </button>
                          ...
                          <div>
                            ...
                            <div className="assignment-card__aux-actions">
                              <Button
                                variant="ghost"
                                size="sm"
                                className="assignment-card__details-button"
                                disabled={isLoadingDetails}
                                onClick={() => void handleOpenDetails(target)}
                              >
                                {isLoadingDetails ? 'Abrindo...' : 'Ver ocorrência'}
                              </Button>
                              {accepted ? (
                                <Button
                                  variant="secondary"
                          ...
                          <div className="assignment-card__decision">
                            {!isRejecting ? (
                              <div className="assignment-card__decision-actions">
                                <Button disabled={isSaving} onClick={() => void handleAccept(target)}>
                                  {isSaving ? 'Processando...' : 'Aceitar ocorrência'}
                                </Button>
                                <Button variant="danger" disabled={isSaving} onClick={() => openReject(target.targetId)}>
                                  Recusar
                                </Button>
                              </div>
                            ) : (
                              <div className="assignment-card__reject-form">
                                ...
                                <div className="assignment-card__decision-actions">
                                  ...
                                  <Button variant="secondary" disabled={isSaving} onClick={cancelReject}>
                                    Cancelar
                                  </Button>
                                </div>
                              </div>
        ...
        {selectedOccurrence ? (
          <PublicOccurrenceDetailsModal occurrence={selectedOccurrence} onClose={() => setSelectedOccurrence(null)} />
        ) : null}

        {selectedChat ? (
          <OccurrenceChatModal
            targetId={selectedChat.targetId}
            publicCode={selectedChat.publicCode}
